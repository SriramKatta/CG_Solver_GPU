...
bool compareArrays(const tpe *a, const tpe *b, size_t n,
                   tpe tolerance = 1e-12) {
...
int main(int argc, char **argv) {
  ...
  for (int test_app = 0; test_app <= 1; ++test_app) {
    ...
    initConjugateGradientDistributed(u_dist.data(), rhs_dist.data(), nx, ny,
                                     comm);
    ...
    MPI_Gatherv(u_send, local_ny * nx, MPI_DOUBLE, u_gathered.data(),
                recvcounts.data(), displs.data(), MPI_DOUBLE, 0,
                MPI_COMM_WORLD);

    MPI_Gatherv(rhs_send, local_ny * nx, MPI_DOUBLE, rhs_gathered.data(),
                recvcounts.data(), displs.data(), MPI_DOUBLE, 0,
                MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      ...
      bool rhs_match =
        compareArrays(rhs_serial.data(), rhs_gathered.data(), nx * ny);

      std::cout << "Test app=" << test_app << std::endl;
      std::cout << "  u arrays match: " << (u_match ? "YES" : "NO")
                << std::endl;
      std::cout << "  rhs arrays match: " << (rhs_match ? "YES" : "NO")
                << std::endl;
